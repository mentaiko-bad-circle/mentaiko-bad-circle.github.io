...
- GitHub push 失敗時(403): `"$LOCALAPPDATA/gh-cli/bin/gh.exe" auth switch --user mentaiko-bad-circle` → `auth setup-git` → 再push
...
- `data/schedule.js` — schedule config, edit here to update dates
  ...
  - `RECRUITMENT_OPEN`: false にすると全日程の募集を一括停止
  - `full: true` を各エントリに追加 → スケジュールに「満員」バッジ、フォームでキャンセル待ち受付
- `css/style.css` — styles
- `js/main.js` — all JavaScript logic（フォームは mailto: リンクで送信、サーバー不要）
...
- gh auth: push失敗(403)時は `auth switch --user mentaiko-bad-circle` → `auth setup-git` の順で実行
